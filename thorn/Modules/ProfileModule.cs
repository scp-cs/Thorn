...
public class ProfileModule : ModuleBase<SocketCommandContext>
{
    ...
    public ProfileModule(UserAccountsService userAccounts, ILogger<ProfileModule> logger, ConstantsService constants,
    ...
    public async Task SetCommand(AccountItem accountItem, [Remainder] string value)
    {
        ...
        _logger.LogInformation("{ContextUser} changed their profile setting {AccountItem} to: {Value}", Context.User,
            accountItem, value);
        await _console.SendMessageAsync(embed: GetProfileUpdateEmbed(
            $"**`{Context.User}`** změnil položku **{accountItem}** na: `{value}`", Context.User as IGuildUser, true));
    }
    ...
    public async Task RemoveCommand(AccountItem accountItem)
    {
        ...
        await _console.SendMessageAsync(embed: GetProfileUpdateEmbed(
            $"**`{Context.User}`** z profilu odstranil **{accountItem}**", Context.User as IGuildUser, false));
    }
    ...
    {
        return Regex.Match(link, @"^http:\/\/(scp-cs(-sandbox(-2|)|)\.wikidot\.com|scp-wiki\.cz)\/[^\s]*$").Success;
    }
    ...
    private static Embed GetProfileUpdateEmbed(string change, IGuildUser user, bool set)
    {
