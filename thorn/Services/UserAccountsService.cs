...
public class UserAccountsService
{
    ...
    {
        await _data.SaveAllUserAccounts(_accounts, FilePath);
    }
    ...
    public async Task<UserAccount> GetAccountAsync(SocketUser user)
    {
        return await GetAccountAsync(user.Id);
    }

    public async Task<UserAccount> GetAccountAsync(ulong id)
    {
        return await GetOrCreateAccount(id);
    }
    ...
    {
        return _accounts.OrderByDescending(x => x.Points.Sum(y => y.Value)).ToList();
    }
